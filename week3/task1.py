...
with open("hotels.csv", "w", encoding="utf-8", newline="") as csvFile:
    ...
    for i in range(len(innOrderChinese)):
        ...
        writer.writerow([innOrderChinese[i]["旅宿名稱"], innOrderEnglish[i]["hotel name"], innOrderChinese[i]["地址"], innOrderEnglish[i]["address"], innOrderChinese[i]["電話或手機號碼"], innOrderChinese[i]["房間數"]])
...
with open("districts.csv", "w", encoding="utf-8", newline="") as csvFile:
    writer = csv.writer(csvFile)
    for innName in district_hotels.keys():
        #str = f"{innName},{district_hotels[innName]},{district_rooms[innName]}\n"
        writer.writerow([innName, district_hotels[innName],district_rooms[innName]])
